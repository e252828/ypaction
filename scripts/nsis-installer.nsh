...
!macro customInit
  CreateDirectory "$APPDATA\YP Action"
  FileOpen $9 "$APPDATA\YP Action\install-timing.log" w
  ...
  ;
  ; 1. YP Action.exe — the main app AND the OpenClaw gateway (ELECTRON_RUN_AS_NODE)
  ; 2. node.exe whose binary lives inside the YP Action install tree
  ...
  DetailPrint "[Installer] Stopping running YP Action processes"
  System::Call 'kernel32::GetTickCount()i .r7'
  nsExec::ExecToLog 'powershell -NoProfile -NonInteractive -Command "\
    Stop-Process -Name 'YP Action' -Force -ErrorAction SilentlyContinue;\
    Get-Process node -ErrorAction SilentlyContinue | Where-Object { $$_.Path -like \"'*YP Action*'\" } | Stop-Process -Force -ErrorAction SilentlyContinue;\
    for ($$i = 0; $$i -lt 15; $$i++) {\
      $$procs = @();\
      $$procs += Get-Process -Name 'YP Action' -ErrorAction SilentlyContinue;\
      $$procs += Get-Process node -ErrorAction SilentlyContinue | Where-Object { $$_.Path -like \"'*YP Action*'\" };\
  ...
  IntOp $5 $6 - $7
  FileOpen $9 "$APPDATA\YP Action\install-timing.log" a
  !insertmacro GetTimestamp $8
  ...
  ; Copy non-bundled skills to %APPDATA%\YP Action\skills-backup\ so they are
  ...
  System::Call 'kernel32::GetTickCount()i .r7'
  ClearErrors
  FileOpen $R0 "$APPDATA\YP Action\skill-migrate.log" w
  ...
  nsExec::ExecToStack 'powershell -NoProfile -NonInteractive -Command "\
    ...
    $$backup = \"$APPDATA\YP Action\skills-backup\";\
...
!macro customInstall
  ...
  ; Log file: %APPDATA%\YP Action\install-timing.log

  CreateDirectory "$APPDATA\YP Action"
  FileOpen $2 "$APPDATA\YP Action\install-timing.log" a
  !insertmacro GetTimestamp $8
  ...
  nsExec::ExecToLog '"$INSTDIR\${APP_EXECUTABLE_FILENAME}" "$INSTDIR\resources\unpack-cfmind.cjs" "$INSTDIR\resources\win-resources.tar" "$INSTDIR\resources" "$APPDATA\YP Action\install-timing.log"'
  Pop $0
  System::Call 'kernel32::GetTickCount()i .r6'
  IntOp $5 $6 - $7
  ...
  TarExtractProcessFailed:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "Resource extraction failed: could not start extractor process (exit=$0). This may be caused by antivirus software. See %APPDATA%\YP Action\install-timing.log for details."
    Goto TarExtractOK
  ...
  TarExtractNonZero:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "Resource extraction failed (exit code $0). See %APPDATA%\YP Action\install-timing.log for details."
  TarExtractOK:
  ...
  IfFileExists "$APPDATA\YP Action\skills-backup\*.*" 0 SkipSkillRestore
...
!macro customUnInit
  ...
  ; (also named YP Action.exe) and shows an "app cannot be closed" dialog
