...
/**
 * Apply version-specific YP Action patches to the openclaw source tree.
 *
...
// This removes stale patches left by a different YP Action branch that may have
...
for (const patchFile of patchFiles) {
  ...
  if (needsNormalize) {
    patchPath = path.join(os.tmpdir(), `YP Action-patch-${patchFile}`);
