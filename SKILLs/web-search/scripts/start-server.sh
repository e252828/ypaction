...
resolve_node_runtime() {
  ...
  if [ -n "${YPACTION_ELECTRON_PATH:-}" ] && [ -x "${YPACTION_ELECTRON_PATH}" ]; then
    NODE_CMD="$YPACTION_ELECTRON_PATH"
    NODE_ARGS=()
...
ensure_npm_available() {
  ...
  echo "  Please reinstall the web-search skill runtime from YP Action."
  return 1
}
...
if ! resolve_node_runtime; then
  ...
  echo "  Please install Node.js, or run from YP Action so scripts can use Electron runtime."
  exit 1
fi
