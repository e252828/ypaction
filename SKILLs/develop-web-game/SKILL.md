...
```bash
export SKILLS_ROOT="${YPACTION_SKILLS_ROOT:-${SKILLS_ROOT:-$HOME/Library/Application Support/YP Action/SKILLs}}"
...
Installed skills resolve from `$YPACTION_SKILLS_ROOT` / `$SKILLS_ROOT` (production default: app `userData/SKILLs`, macOS usually `~/Library/Application Support/YP Action/SKILLs`).
