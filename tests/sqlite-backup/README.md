...
2. 确认目标数据库路径。默认桌面版用户数据路径通常是：  
   ...
   ```bash
   ~/Library/Application\ Support/YP Action/ypaction.sqlite
   ```
...
npm run test:sqlite-backup:seed -- \
  --db '/Users/jj.deng/Library/Application Support/YP Action/ypaction.sqlite' \
  --sessions 10 \
...
```bash
YPACTION_SQLITE_BACKUP_ALWAYS_ON_STARTUP=1
```
...
```bash
YPACTION_SQLITE_BACKUP_ALWAYS_ON_STARTUP=1 npm run electron:dev
```
...
- `[SqliteBackup] Starting periodic backup to YP Action-latest.sqlite`
...
```bash
ls -lh ~/Library/Application\ Support/YP Action/backups/sqlite/snapshots/
```
...
```bash
printf 'not-a-sqlite-db' > ~/Library/Application\ Support/YP Action/ypaction.sqlite
```
...
4. 使用 `YPACTION_SQLITE_BACKUP_ALWAYS_ON_STARTUP=1` 启动应用。  
   Launch the app with `YPACTION_SQLITE_BACKUP_ALWAYS_ON_STARTUP=1`.
5. 记录备份开始、进度、完成日志及耗时。  
